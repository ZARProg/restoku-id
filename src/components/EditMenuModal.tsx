...
interface EditMenuModalProps {
  ...
  onClose: () => void;
  menu: MenuItem | null;
  onSave: (menuId: string, updatedData: Omit<MenuItem, 'id'>) => void;
}

const EditMenuModal: React.FC<EditMenuModalProps> = ({ isOpen, onClose, menu, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!menu || !formData.name.trim() || !formData.description.trim() || formData.price <= 0) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Harga * <span className="text-xs text-gray-500">({formatCurrency(formData.price)})</span>
            </label>
            <input
              type="number"
              min="0"
